             ...
             <Accordion type="single" collapsible className="w-full space-y-4">
              {items.map((item, index) => (
                <AccordionItem 
                  key={index} 
                  value={`item-${index}`} 
                  className="border border-border rounded-2xl px-6 bg-card shadow-sm transition-all group data-[state=open]:border-accent/40"
                >
                  <AccordionTrigger className="text-left text-lg md:text-xl font-semibold text-foreground hover:text-accent transition-colors hover:no-underline py-6 text-balance">
                    {item.question}
                  </AccordionTrigger>
